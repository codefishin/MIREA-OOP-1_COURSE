...
std::string cl_base::get_abs() // доп. метод, получение абсолютного пути
{
  cl_base * temp = p_get_head_object();
  std::string s_abs = "";
  if (temp == nullptr)
  {
    s_abs = "/"; // если мы уже на вершине
    return s_abs;
  }
  s_abs = '/' + s_get_object_name();
  while (temp->p_get_head_object() != nullptr)
  {
    s_abs = '/' + temp->s_get_object_name() + s_abs;
    temp = temp->p_get_head_object(); // эта строка будет в моих кошмарах
  }
  return s_abs; // вернуть абсолютный путь
}
void cl_base::set_all_state() // доп. метод, включает все объекты
{
  state = 1; // поставить вкл
  for (auto current : subordinate_objects) // пробегает все подчинённые
  {
    current->set_all_state(); // поставить всё на вкл
  }
}
void cl_base::emit_signal(SIGNAL_TYPE signal, std::string message)
{
  // Сигналы :D
  if (state == 0) return;
  (this->*signal)(message);
  // "Этому" сигналу передаём значение параметра message
  for (auto connection : connections)
  {
    // Поиск сигнала
    if (connection->signal == signal && connection->target->state != 0)
    {
      // Найден! Получить сообщение...
      cl_base * p_target = connection->target; // Наш target
      HANDLER_TYPE p_handler = connection->handler; // Наш handler
      (p_target->*p_handler)(message); // Получение сообщения
    }
  }
}
void cl_base::set_connection(SIGNAL_TYPE signal, cl_base * target, HANDLER_TYPE handler)
{
  // Метод для установки сигнала
  o_sh * p_value; // Структурное сигнала
  for (int i = 0; i < connections.size(); i++) // Пробегаем все элементы сигналов
  {
    // Проверка прочих соединений
    if (connections[i]->signal == signal &&
    connections[i]->handler == handler &&
    connections[i]->target == target)
    {
      return; // Сигнал уже есть похожий на тот, который вызывается, ничего не делать
    }
  }
  p_value = new o_sh();
  // Замена конструктору. Самостоятельно выполняем инициализацию. Опасно!
  p_value->signal = signal;
  p_value->handler = handler;
  p_value->target = target;
  connections.push_back(p_value); // Добавление сигнала
}
void cl_base::delete_connection(SIGNAL_TYPE signal, cl_base * target, HANDLER_TYPE handler)
{
  // Метод для удаления сигнала
  for (int i = 0; i < connections.size(); i++)
  {
    ...
    connections[i]->target == target)
    {
      connections.erase(connections.begin() + i); // удалить коннект
    }
  }
}
